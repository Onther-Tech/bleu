...
- bleu-daemon uses the url of the json file in the [`task`](./task) folder. Open all the json files inside the [`task`](./task) folder and check the settings.
...
- Currently, three urls are used for the json file in the task folder.
  ...
  // l1_tx_log.json
  {
    "l1_tx_log": {
      "start_idx": 0,
      "end_points": [
        "https://rinkeby.rpc.tokamak.network"
      ],
      "filter": ""
    }
  }
  ...
  // l2_block_tx.json
  {
    "l2_block_tx": {
      "start_idx": 0,
      "end_points": [
        "https://testnet3.optimism.tokamak.network"
      ],
      "filter": ""
    }
  }
  ...
  // l2_enqueue.json
  {
    "l2_enqueue": {
      "start_idx": 0,
      "end_points": [
        "http://testnet3.optimism.tokamak.network:7878/enqueue/index/"
      ],
      "filter": ""
    }
  }
  ...
  // l2_state_batch.json
  {
    "l2_state_batch": {
      "start_idx": 0,
      "end_points": [
        "http://testnet3.optimism.tokamak.network:7878/batch/stateroot/index/"
      ],
      "filter": ""
    }
  }
  ...
  // l2_tx_batch.json
  {
    "l2_tx_batch": {
      "start_idx": 0,
      "end_points": [
        "http://testnet3.optimism.tokamak.network:7878/batch/transaction/index/"
      ],
      "filter": ""
    }
  }
...
```json
{
  "l2_block_tx": {
    "start_idx": 0,
    "end_points": ["http://localhost:8545"],
    "filter": ""
  }
}
```
...
In this case, retry-endpoint, which is an endpoint for retry requests, can be set in [config.toml](./config.toml).

### Polling Interval

The Loop Polling task can adjust the synchronization speed by adjusting the `poll-interval` value in [config.toml](./config.toml).
...
The Trigger task can adjust the number of retries by adjusting the `retry-count` value in [config.toml](./config.toml).
...
These values can be entered through [config.toml](./config.toml).
...
3. enter webhook url into [config.toml](./config.toml)
...
To activate the slack plugin, you need to change slack activate in [config.toml](./config.toml) to true.
...
Various configuration values required to run the Bleu Daemon are managed in [config.toml](./config.toml).
These values can also be entered in the form of `--jsonrpc-host 0.0.0.0` at run time.
The path of [config.toml](./config.toml) is located in `~/.config/bleu-damon/config`, but the path has been modified so that the project root path can be used in the following executable statements and docker.
