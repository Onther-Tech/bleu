...
import { Avatar, Box, Grid, Link, Table, TableBody, TableCell, TableRow, Typography } from '@mui/material';
...
function LatestL1Batches() {
  ...
  return (
    <InfoCard
      ...
      buttonProps={{ label: 'View all Transaction batches', href: '/blocks' }}
      sx={{ height: '500px' }}
    >
      <Table>
        <TableBody>
              ...
              <TableCell sx={index === state.length - 1 ? tableCellLast : tableCell}>
                <Grid container spacing={1}>
                  ...
                  <Grid item lg={6} md={6} sm={12} xs={12} sx={content}>
                    ...
                    <Link variant="body2" underline="none" href={`/txs?blockNum=${row.batch_index}`}>
                      {row.batch_size} txns
                    </Link>
                  </Grid>
                </Grid>
